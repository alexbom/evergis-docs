...
### Подстановки в строках конфига

Все подстановки разбирает один резолвер (`utils/configString`, одна грамматика токенов). Он работает в трёх режимах: параметры запроса, условие слоя, текст интерфейса.

| Токен | Откуда значение |
|---|---|
| `%name`, `%name.min` / `.max`, `%name.lN`, `%name.<prop>` | фильтр **своего** виджета; в карточке — фильтр карточки |
| `$dashboard:name` (те же свойства) | фильтр **ближайшего дашборда** (`DashboardContext.filters`); пока его не трогали — `defaultValue` фильтра дашборда. На самом дашборде `$dashboard:name` равен `%name` |
| `$dashboard:<датасорс>:<поле>` | поле **первой строки датасорса дашборда**: ищется по имени, затем по `layerName`. Пока датасорс не загружен, значение пустое. Заменяет снятый `$left:<слой>:<поле>` |
| `{attributeName}` | атрибут текущего объекта (у строк `DataSource` — атрибут строки) |
| `$card:<слой>:<поле>` | атрибут объекта карточки, если карточка открыта на этом слое |
| `%geometry`, `%extent`, `%zoom`, `%project[.name\|.alias]` | системные значения, см. выше |

Свойства: `.min` / `.max` — границы диапазона, `.lN` — уровень tree-фильтра, любое другое `.prop` — свойство строки `relatedDataSource`, выбранной фильтром.

Формы `$dashboard:` различает второе двоеточие: `$dashboard:year.min` — фильтр со свойством, `$dashboard:em.roads:code` — датасорс и поле. Имя датасорса или слоя — буквы, цифры, `_`, `.` и `-`.

Граница имени — слово целиком: `%x` не цепляет `%x2`, точка в конце предложения («за %year.») не считается свойством. Заменяются **все** вхождения токена.

**Миграция с `$left`.** `$left:<слой>:<поле>` больше не подстановка — строка уходит литералом. Замена один в один: `$dashboard:<слой>:<поле>`.

Незнакомый токен остаётся в строке как есть: опечатка, литерал `50%off`, `{что-то}` без такого атрибута, `$dashboard:` без такого фильтра.

**Где работает:**

| Режим | Поля | Строка-токен целиком | Токен внутри строки | Пустое значение |
|---|---|---|---|---|
| Параметры запроса | `parameters` датасорса, задачи, хука сохранения | значение с типом (число, массив, объект `features`) | текстом | ключ не уходит |
| Условие | `condition` датасорса и слоя | — | в формате условия: кавычки по типу, списки `[…]`, даты `#'…'` | плейсхолдер остаётся |
| Текст интерфейса | `value`, `defaultValue` элемента; `options.label`, `options.placeholder`, `options.title`; заголовок модалки | значение с типом | массив через `", "` | пустая строка — срабатывают `defaultValue` и `hideEmpty` |
| Дефолт фильтра карточки | `filters[].defaultValue` | только `$dashboard:` | только `$dashboard:` | дефолта нет |

```json
{ "id": "alias", "value": "Показатели за $dashboard:year год" }
{ "id": "scenario", "type": "control", "defaultValue": "$dashboard:scenario" }
{ "name": "year", "defaultValue": "$dashboard:year" }
```

**Когда значение обновляется:**

- Тексты и подписи **живые**: сменили фильтр дашборда — сменилась подпись.
- `defaultValue` контрола берётся **один раз**: первое непустое значение после открытия карточки. Смена фильтра дашборда не затирает выбор. В `controls` дефолт не пишется, это только показ.
- `defaultValue` фильтра карточки из `$dashboard:` живой, пока фильтр карточки не выбран. После ручного выбора остаётся выбор пользователя.
- Датасорсы карточки перезапрашиваются при смене фильтра дашборда, если их `parameters` ссылаются на него: напрямую через `$dashboard:name` или через `%name` фильтра карточки с дефолтом `$dashboard:`. Датасорсы слоя без `query` перезапрашиваются на любое изменение, как и раньше.

Утилиты: `applyQueryFilters` (параметры) и `formatDataSourceCondition` (условие) — обёртки над резолвером с прежними сигнатурами. Для строки UI — `resolveConfigString`, источники — хук `useConfigStringSources(type)`.
...
> - У `StructuredData` сверх слотов проверяется собственный набор инвариантов (`validateStructuredData.ts`): есть ребёнок `data` с `type: "table"` (`missing-view`); задан `options.filterName` (`missing-filter-name`), и такой фильтр объявлен на странице с `valueType: "features"` (`invalid-filter-value-type`); без `relatedDataSource` описана схема (`missing-schema`); имена атрибутов уникальны (`duplicate-attribute`); `options.saveToLayer` стоит только на источнике со слоем — без `relatedDataSource` или на источнике, объявленном без `layerName`, это `save-to-layer-without-layer`, а без `editMode` — предупреждение `orphan-option`. Списки значений колонок проверяет `validateStructuredDataLists.ts`: неизвестный `control` — ошибка `invalid-control`; `control` без списка, `attributeValue`/`attributeAlias` без `relatedDataSource`, `variants` вместе с `relatedDataSource` и список у колонки цвета или вложений — предупреждения `orphan-option`; справочник, не объявленный среди `dataSources` конфига, — предупреждение `unknown-list-source` (предупреждение, а не ошибка: слой страницы редактор регистрирует источником только при сохранении).
