...
**Поля `ConfigAttributeDescription`:** `attributeName` (обязателен), `type`, `subType`, `alias`, `description`, `isEditable`, `stringFormat`, `width`, `resizable`, `multiline`, `style`. Первые шесть повторяют форму `AttributeConfigurationDc` из `@evergis/api`, остальные — раскладка и вид колонки, которых в серверном контракте нет.
...
**Вложения в колонке (`subType: "Attachments"`).** Отдельного `AttributeType` под вложения в API нет: файлы приезжают строкой с массивом JSON, и разбирать её как список вложений велит именно `subType`. Тип атрибута при этом остаётся `"String"`. Ячейка показывает файлы списком — значок типа файла и имя, клик открывает просмотр или скачивание; с `editMode` и `isEditable` рядом появляются удаление и добавление по ссылке. Загрузки с диска в колонке нет: она требует ресурса-родителя и файлового api, которые есть у [[containers#AttachmentContainer|контейнера вложений]], но не у колонки таблицы. Подробности — в [[elements#ElementTable|ElementTable]].
