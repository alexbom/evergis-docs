...
Три slot-id допустимы у **любого** контейнера независимо от его собственного набора, поэтому в карточках ниже отдельно не перечисляются:

| Slot-id | Кто рендерит | Где рисуется | Ограничение |
|---|---|---|---|
| `title` | [[components\|`ExpandableTitle`]] | заголовок контейнера | нужен контейнер с заголовком |
| `titleIcon` | там же | иконка заголовка | там же |
| `bgImage` | [[components\|`ContainerBackground`]] | абсолютный слой **под** содержимым | у любого контейнера, **кроме `Divider`** |

Все три контейнер читает по `id` сам: `ContainerChildren` исключает их из рендера тела, а сетка — из подсчёта треков (`NON_TRACK_SLOT_IDS`). Слот `icon` на уровне контейнера тоже уходит в заголовок, но, в отличие от этих трёх, входит в наборы слотов конкретных контейнеров.

`bgImage` — обычный элемент `type: "image"`: URL берётся из корневого `value`, `attributeName` или `options.resourceId`, а `options.fit` перебивает дефолтный `object-fit: cover` слоя. Пока слот не объявлен, в DOM не появляется ничего — ни обёртки, ни пустого слоя ([[utils|`hasContainerBgImage`]]). Корень контейнера при этом становится хостом слоя: `position: relative` + `isolation: isolate`. Механика целиком — в [[concepts#Универсальные слоты и фон контейнера|Основных понятиях]].

### Опции-компаньоны фона

Две опции из [[options#ConfigLayoutOptions|`ConfigLayoutOptions`]] тоже допустимы у **любого** контейнера и в таблицах опций ниже не повторяются: ни один `<Name>Options` их не Pick'ает — значения читают хук хоста и слой фона напрямую из `options`.

| Опция | Тип | Описание |
|---|---|---|
| `innerPadding` | `boolean` | Внутренний отступ `1rem` по всем краям корня контейнера (`CONTAINER_INNER_PADDING`), чтобы содержимое не липло к краям картинки. Ставится под селектором `&&` — перебивает `padding` из `$sizeCss` и внутренних `defaults`, но не авторский inline-`style`. Работает и **без** фона: гейт отдельный от `$hasBgImage` |
| `outflow` | `boolean` | Слой `bgImage` вытекает за края контейнера на `1.5rem` (`BG_IMAGE_OUTFLOW`) — по бокам и вверх, вниз никогда. Раскладку не меняет: слой абсолютный, содержимое остаётся в своих границах. Без слота `bgImage` бессмысленна (валидатор отдаёт `orphan-option`); обрезается любым предком с `overflow` кроме `visible`, включая собственный `options.overflow` |
...
- [[hooks|хук]] `useAttachmentDownload` — кнопка «Скачать» в просмотрщике

**Скачивание.** Кнопка «Скачать» просмотрщика (`Preview.onDownload`) тянет файл в момент клика, а не заранее: свой — через `api.catalog.getFile` с сохранением из памяти ([[utils|`saveBlobAsFile`]]), внешний — открытием ссылки в новой вкладке. Ошибка запроса уходит в уведомление. Тот же механизм включён и в `EditAttachmentContainer`.
...
| `legendInline` | `boolean` | Легенда встаёт в одну строку справа от подписи оси X. Игнорируется при `twoColumns`, при незаданной подписи оси X и у нелинейных графиков (подписи оси X у них нет) |
...
#### Подпись оси X и легенда

Подпись оси X задаётся у элемента `chart` (`options.axis.x.title`), но строку под графиком строит **контейнер** — только он видит одновременно слот `chart` и слот `legend`.

> [!note] Только у линейного графика
> Подпись оси X рисуется только при `chartType: "line"`. У других типов `axis.x.title` игнорируется, даже если остался в конфиге после смены типа графика: редактор показывает настройки осей только линейному графику, и убрать такую подпись там было бы нечем. Вместе с подписью у других типов не работает и `legendInline` — легенда раскладывается как раньше.

- строка с подписью и легенда под ней выравниваются **по полю графика** (области линий и сетки), а не по краям контейнера — как на макете. Где лежит поле, знает только d3 после отрисовки, поэтому элемент `chart` сообщает его отступы контейнеру через контекст `ChartPlotContext`. До первого отчёта обе строки скрыты (`visibility: hidden`, место сохраняется): иначе на первом кадре они стояли бы по краю контейнера и затем прыгали на место;
- по умолчанию легенда идёт **отдельной строкой под подписью оси X**, в 8px под ней;
- `legendInline: true` ставит её **в ту же строку, справа** — единственное исключение из правила;
- когда подпись оси X задана, легенда на своей строке выравнивается по `options.axis.x.align` элемента `chart`;
- у линейного графика без подписи оси X легенда встаёт на её место: в 4px под подписями делений, по левой границе поля графика (по центру — при опции легенды `center`). У остальных типов графиков легенда раскладывается по-прежнему.

Слот `legend` рендерится ровно один раз при любом сочетании `legendInline` × `twoColumns`. При легенде сбоку (`twoColumns`) подпись оси X остаётся под графиком — в одной колонке с ним.

Строка с подписью подтянута к подписям делений оси X: под ними внутри SVG остаётся пустая полоса (`LINE_CHART_X_LABELS_BLANK`), и без поправки зазор получился бы втрое больше макетных 4px.

Подписи самих осей Y рисует элемент `chart` — см. [[elements#ElementChart|ElementChart]].
...
- **По высоте** фит включается только при заданной `height` контейнера. Тогда `chartHeight` означает не «высоту тела», а **всю доступную высоту ячейки**, из которой вычитается обвес: подписи оси X и внешний отступ у BarChart, строка итога у StackBar (её высота не прибавляется, а забирается флексом), строка горизонтальных подписей осей у линейного графика. Круглый график (`pie`) вписывается по короткой стороне и центрируется.
- **Выступы подписей шкал.** `ChartFillMeasure` режет всё, что выходит за SVG (`overflow: hidden`), поэтому у линейного графика под выступающие подписи резервируются поля: снизу под подписи оси X (`LINE_CHART_LABEL_MARGIN`), сверху под верхнее деление шкал, которое d3 центрирует на верхней линии сетки (`Y_AXIS_LABEL_OVERHANG`). Без верхнего резерва значения верхнего деления обеих осей срезались наполовину.
  ...
  children: [
    { id: "alias", value: "Распределение по типам" },
    {
      ...
      options: {
        chartType: "pie",
        relatedDataSources: [
          { dataSourceName: "types_ds", attributeName: "count", attributeTitle: "type", axis: { type: "y" } }
        ]
      }
    },
    { id: "legend", type: "legend", options: { chartId: "chart" } }
...
Линейный график с двумя шкалами, подписями осей и легендой в строке с подписью оси X:

```tsx
{
  id: "capex_chart",
  templateName: "Chart",
  options: { fill: true, height: 200, legendInline: true },
  children: [
    { id: "title", type: "text", value: "Затраты (CAPEX+OPEX)" },
    {
      id: "chart",
      type: "chart",
      options: {
        chartType: "line",
        axis: { titlePosition: "top", x: { title: "Годы расчёта значений", align: "left" } },
        relatedDataSources: [
          {
            dataSourceName: "revenue_ds", attributeName: "value", attributeTitle: "year", alias: "Выручка",
            axis: { side: "left", color: "#e6007e", title: "млн, в валюте расчёта" }
          },
          {
            dataSourceName: "carbon_ds", attributeName: "value", attributeTitle: "year", alias: "Выработка УЕ",
            axis: { side: "right", color: "#6cc24a", title: "Выработка УЕ в год" }
          }
        ]
      }
    },
...
Исключение — [[containers#Универсальные слоты|универсальные слоты]]: `title` / `titleIcon` / `bgImage` принадлежат самому хосту, а не записи. Слот фона `DataSourceInnerContainer` **вырезает** из конфига записи: иначе одна картинка нарисовалась бы и у хоста, и у каждой из N записей.
...
Перед рендером записи из `elementConfig` вырезается слот `bgImage`: у DataSource-хостов дети — слоты внутреннего шаблона, поэтому фон достался бы и хосту, и каждой записи. Фон остаётся за хостом — он и есть контейнер.
...
> [!warning] Единственный контейнер без слота `bgImage`
> У разделителя нет ни бокса, ни собственного содержимого — одна линия, и слой фона там молча потерялся бы. `Divider` — единственное исключение из [[containers#Универсальные слоты|универсальных слотов]]; клиентский валидатор отдаёт на такой узел ошибку `unexpected-slot`.
...
| Опция | Тип | Описание |
|---|---|---|
| `attributesDescription` | `ConfigAttributeDescription[]` | Схема данных, она же набор колонок таблицы: что описано, то и показано, в порядке описания. С источником — переопределение его схемы: задаёт состав, порядок, `alias`, `stringFormat`, `isEditable`, а также раскладку и вид колонки (`width`, `resizable`, `multiline`, `style`, `subType`); типы приходят от источника. Без источника — единственная схема, обязательна |
...
| `editMode` | `boolean` | Разрешить правку таблицы: редакторы в ячейках, добавление и удаление строк, панель отката и записи. Не задан или `false` — таблица только на чтение. Та же опция, что у сетки [[containers#ContainersGroupContainer\|ContainersGroup]], но читает её этот контейнер сам. Состав строк закрывается отдельно — опцией `editOnly` у ребёнка `data` |
| `saveToLayer` | `boolean` | Писать правку в слой источника через features-API. По умолчанию `false`: «Сохранить» пишет только в фильтр. В фильтр результат уходит **всегда**, с опцией или без. Работает только с `editMode` и источником со слоем (`layerName`) — см. «Запись в слой» ниже |
| `expandable` | `boolean` | Разрешить сворачивание |
| `expanded` | `boolean` | Развёрнут ли по умолчанию |

Плюс `ContainerBoxOptions` (`width`, `height`, `overflow`).

**Поля `ConfigAttributeDescription`:** `attributeName` (обязателен), `type`, `subType`, `alias`, `description`, `isEditable`, `stringFormat`, `width`, `resizable`, `multiline`, `colorPicker`, `style`, `parentResourceId`, `fileExtensions`, `control`, `variants`, `relatedDataSource`, `attributeValue`, `attributeAlias`. Первые шесть повторяют форму `AttributeConfigurationDc` из `@evergis/api`, остальные — раскладка, вид и настройки колонки, которых в серверном контракте нет.

| Поле | Тип | Описание |
|---|---|---|
| `subType` | `StringSubType` | Уточнение строкового типа. `"Attachments"` превращает колонку в список вложений — см. ниже |
| `width` | `number` | Ширина колонки в пикселах. Не задана — ширину выбирает раскладка таблицы по содержимому |
| `resizable` | `boolean` | Ширину колонки можно тянуть мышью за правую границу заголовка. От `editMode` не зависит: ширина — это вид, а не данные |
| `multiline` | `boolean` | Значение переносится по словам вместо обрезки многоточием, высота строки растёт под содержимое |
| `colorPicker` | `boolean` | Строковое значение — цвет (`hex`, `rgb` или `rgba`): колонка показывает плашку этого цвета рядом со значением, а правится палитрой — см. ниже |
| `style` | `CSSProperties` | Стиль значений колонки (семейство, размер, цвет и начертание шрифта). Своего узла-элемента у колонки нет, поэтому стиль живёт прямо у описания атрибута |
| `parentResourceId` | `string` | Папка каталога, куда уходит файл, загруженный с диска в колонку вложений. Не задан — файл уходит в корень |
| `fileExtensions` | `string` | Что разрешено выбирать в диалоге файлов колонки вложений — значение `accept` (`".pdf,.jpg"` либо `"image/*"`) |
| `variants` | `ChipOption[]` | Допустимые значения колонки: `{ text, value, color?, icon? }` — в ячейку пишется `value`, видна `text`. Заданы вместе с `relatedDataSource` — выигрывают варианты. См. «Список значений в колонке» ниже |
| `relatedDataSource` | `string` | ⚠️ entity-ref: справочник — источник страницы, из записей которого собирается список значений. Обязан быть объявлен в `dataSources` страницы: отдельно его никто не загрузит |
| `attributeValue` | `string` | Атрибут записи справочника, чьё значение пишется в ячейку. Не задан — `attributeName` колонки |
| `attributeAlias` | `string` | Атрибут записи справочника, который виден подписью. Не задан — `attributeValue`. Не путать с `alias` — это заголовок колонки |
| `control` | `"dropdown" \| "chips"` | Контрол выбора из списка (`ConfigAttributeControl`): выпадающий список (по умолчанию) или ряд кнопок. Без списка ни на что не влияет |

**Цвет в колонке (`colorPicker: true`).** Отдельного типа под цвет нет и не нужно: значение остаётся обычной строкой, а читать её как цвет велит сама опция. Разбираются три вида записи — `hex`, `rgb`, `rgba`. Ячейка показывает квадратную плашку этого цвета и рядом само значение; неразбираемая строка остаётся просто текстом, без плашки, — так видно, что в данных мусор, а не пустое значение. С `editMode` и `isEditable` вместо плашки встаёт палитра с ползунком прозрачности, а выбранный цвет уходит в черновик как `#rrggbb` либо, при неполной непрозрачности, как `#rrggbbaa`. С вложениями опция взаимоисключающа: в редакторе это один список, где «Цвет» и «Вложение» стоят рядом.

**Список значений в колонке (`variants` / `relatedDataSource`).** Колонка со списком принимает только значения из него: свободного ввода у неё нет, какой бы тип ни был у атрибута. Список берётся из `variants`, а без них — из записей справочника `relatedDataSource`: значение пункта — атрибут `attributeValue` (по умолчанию `attributeName` самой колонки), подпись — `attributeAlias` (по умолчанию то же, что значение). Так справочник «id → имя» пишет в ячейку id, а показывает имя; повторяющиеся и пустые значения справочника в список не попадают. В ячейку, в фильтр и в слой уходит **значение**, а не подпись. Справочник должен быть источником страницы: рантайм грузит только объявленные `dataSources`, а ещё не загруженный или не найденный справочник даёт пустой список, а не свободный ввод.

Контрол задаёт `control`:

- **`"dropdown"`** (по умолчанию) — вне фокуса ячейка показывает подпись выбранного пункта, на фокус (клик или Tab) встаёт выпадающий список с поиском и сразу раскрывается. Выбор записывает значение и закрывает правку; пустой пункт «—» сверху снимает значение. Без `editMode` или при `isEditable: false` — просто текст подписи.
- **`"chips"`** — ряд кнопок, видимый всегда (как галка у логической колонки): выбор — один клик, клик по выбранному снимает выбор. Не влезающий ряд переносится, и строка растёт вниз. Без права правки ряд остаётся на месте и показывает выбор, но на клики не отвечает. Кнопки — те же, что у [[containers#FiltersContainer|фильтра-чипсов]]: `color`/`icon` варианта красят чип так же.

Значение, которого в списке нет (справочник поменялся, строка пришла из источника), показывается как есть, по `stringFormat` колонки, а не прочерком — так видно, что в данных. Сортировка по заголовку (`sort` у представления) упорядочивает такие колонки по подписям. Списки зависят от загруженных источников, поэтому живут в схеме **представления**, а не в схеме данных: загрузка справочника не пересобирает черновик и не стирает в нём правки. У колонок цвета и вложений списка нет — их редакторы разбираются раньше (валидатор предупреждает `orphan-option`).

**Вложения в колонке (`subType: "Attachments"`).** Отдельного `AttributeType` под вложения в API нет: файлы приезжают строкой с массивом JSON, и разбирать её как список вложений велит именно `subType`. Тип атрибута при этом остаётся `"String"`. Один файл ячейка показывает сама — значок типа и имя, — а несколько схлопывает в строку со значком `link` и счётчиком; полный список открывается в поп-апе, и там же живут переключатель «плитка / список», «Показать ещё» и добавление из трёх источников: с диска, из каталога ресурсов и по ссылке. Куда класть загруженный с диска файл и что разрешено выбирать, задают `parentResourceId` и `fileExtensions` того же описания атрибута. Подробности — в [[elements#ElementTable|ElementTable]].

**Панель действий.** Слева — общая кнопка «Добавить»: серая скруглённая, со значком `feature_add` и подписью, та же, что добавляет файл во [[containers#AttachmentContainer|вложениях]] (`AddButtonRow` из `Dashboard/components/AddButton`). Размеры сняты с макета: высота `1.5rem`, поля `0.625rem`, просвет до подписи `0.375rem`, значок и подпись по `0.875rem`; фон `elementDark`, значок цветом иконки, подпись — `textPrimary` (сам `IconButton` красит подпись заодно со значком, поэтому цвет задан отдельно). Справа, и только пока черновик грязный, — пара `IconButton`: красный крест отката (`kind="close"` с пропом `error`) и галка записи основным цветом (`kind="success"` с пропом `primary`). Та же пара правит объект в карточке (`EditControls`), поэтому подписи ей не нужны — они ушли в `title`. К правому краю пару прижимает её собственный отступ, а не соседняя группа: с `editOnly` кнопки добавления слева нет, а пара всё равно обязана остаться справа. Всем трём кнопкам `tabIndex` задан явно: `IconButton` по умолчанию ставит -1, а по таблице ходят Tab-ом, и с ячеек он обязан доходить до панели.

**Только правка (`editOnly` у ребёнка `data`).** Правка ячеек остаётся, а состав строк закрыт: кнопки «Добавить» и колонки удаления нет. Так живёт таблица, строки которой задаёт источник или значение фильтра, а дашборд уточняет только значения. Опция объявлена у [[elements#ElementTable|представления]], но читает её контейнер — кнопка добавления его, а право удаления он раздаёт представлению вместе с остальным контекстом, чтобы значение разбиралось один раз. Панель при этом появляется только с первой правкой: без кнопки добавления и с чистым черновиком класть в неё нечего, и пустая полоса с отступами не рисуется.

**Черновик и сохранение.** Правки копятся в локальном черновике: ни источник, ни потребители фильтра не узнают о них до нажатия «Сохранить», «Отменить» возвращает черновик к последнему сохранённому состоянию (прецедент — dirty/onSave у [[containers#FiltersContainer|tree-фильтра]]). Порядок записи: с `saveToLayer` — `deleteFeatures` → `updateFeature` → `createFeatures`, затем `changeFilters`; без неё — только `changeFilters`. Ошибка записи в слой фильтр не трогает: потребители не увидят данных, которых слой не принял, а черновик остаётся для повтора. Созданные строки получают `id` из `createdIds` ответа — повторное сохранение обновляет их, а не плодит дубли.

**Что уходит в features-API** (только с `saveToLayer`). Только атрибуты с `isEditable: true`. Нередактируемые поля не передаются вовсе — их значение принадлежит источнику, а у новой строки его и нет. При записи в слой конфиг может правку запретить, но не разрешить там, где слой её не допускает (id-атрибут, вычисляемые поля).
...
**Запись в слой (`saveToLayer`).** Куда уедет результат, решает конфиг, а не вид источника. В фильтр «Сохранить» пишет всегда; в слой через features-API — только при `saveToLayer: true`, и только если у `relatedDataSource` есть `layerName`. У EQL-, python- и url-источников записи нет, и опция у них уходит вхолостую (валидатор конфига считает эту связку ошибкой `save-to-layer-without-layer`). Права правки от опции не зависят: ячейки, добавление и удаление строк.

От неё зависят две вещи сверх адресата:
- **ограничения схемы.** Без записи в слой ограничения источника на `isEditable` (нередактируемый атрибут, вычисляемое поле, id-атрибут) не действуют — они про запись, которой не будет, поэтому запретить колонку может только `attributesDescription`. Это касается и источника-слоя без `saveToLayer`;
- **удаление строки.** Без записи в слой оно убирает строку лишь из значения фильтра, и следующая загрузка источника вернёт её обратно. Предупреждение о необратимом удалении объекта показывается только при записи в слой.

> [!warning] Ломающее изменение
> До появления `saveToLayer` таблица на источнике-слое писала в слой автоматически. Теперь по умолчанию — только в фильтр: существующим конфигам, которые должны писать в слой, нужно явно добавить `saveToLayer: true`.
...
**Назначение:** Голосование, привязанное к объекту карты. Контейнер работает **только в карточке объекта** (`WidgetType.FeatureCard`): он читает `question_id` из атрибута выбранного объекта и по данным выбирает один из четырёх экранов — создание, голосование, после голосования, только результаты. Все чтения и записи идут напрямую через `api.layers.*`, минуя механику [[concepts#Источники данных|источников данных]] страницы.

**Типы:** `templateName = "Vote"` · `VoteContainerOptions` · `VoteContainerProps`.
...
| Опция | Тип | Описание |
|---|---|---|
| `categoryDataSource` | `string` | Слой-справочник категорий: поля `id`, `name`. Категории только выбираются — создавать их контейнер не умеет |
| `questionDataSource` | `string` | Слой голосований: `id`, `text`, `category_id`, `user_name` (владелец), `multi_select`. Нужны права на запись |
| `variantDataSource` | `string` | Слой вариантов ответа: `id`, `question_id`, `text`. Нужны права на запись |
| `answerDataSource` | `string` | Слой ответов: `id`, `question_id`, `user_name`, `variant_id`. Нужны права на запись |
| `expandable` | `boolean` | Разрешить сворачивание |
| `expanded` | `boolean` | Развёрнут ли по умолчанию |

Плюс `ContainerBoxOptions` (`width`, `height`, `overflow`).

> [!warning] Четыре опции — имена СЛОЁВ, а не источников данных страницы
> Контейнер обращается к ним через `api.layers.getFeatures` / `createFeatures` / `updateFeature` / `deleteFeatures`, поэтому имя из `dataSources` страницы он не найдёт. Ставьте системные имена ресурсов-слоёв, и у слоёв голосований, вариантов и ответов должны быть права на запись. Схема полей жёсткая — она захардкожена в `VOTE_FIELDS` контейнера, конфигом не переопределяется.

**Четыре экрана** выбираются по данным, а не конфигом:

| Экран | Условие | Что показано |
|---|---|---|
| Создание | атрибут объекта пуст, пользователь авторизован | категория из справочника, текст вопроса, варианты-чипсы (минимум два), переключатель множественного выбора, кнопка «Создать голосование» |
| Голосование | голосование есть, пользователь ещё не голосовал | вопрос, варианты с Radio (`multi_select: false`) или Checkbox (`true`), счётчики и прогресс-бары, кнопка «Отправить ответ», блок «Поделиться» |
| После голосования | пользователь уже голосовал | те же результаты без контролов выбора и зелёная плашка «Ваш ответ принят» |
| Только результаты | пользователь не авторизован | результаты и подпись «Чтобы проголосовать, пожалуйста, авторизуйтесь» |
...
**Подсчёт результатов** — клиентский, из одного запроса к слою ответов с условием `question_id == <id>`: счётчики по вариантам, доля голосов варианта от **всех** поданных по голосованию голосов (это и есть заполнение прогресс-бара) и проверка «голосовал ли я» — всё из одного и того же списка.

**Привязка к объекту.** При создании контейнер пишет запись вопроса (владельцем становится текущий пользователь), затем записи вариантов с `question_id` из `createdIds`, и только потом — сам `question_id` в атрибут объекта карты через `updateFeature`. Значит, слой объекта тоже должен принимать запись в этот атрибут. При удалении голосования атрибут очищается — «осиротевших» записей не остаётся.

**Заморозка после первого ответа.** Править (категорию, вопрос, состав и текст вариантов, множественный выбор) и удалять голосование может **только владелец** (`user_name` записи) и **только пока на голосование нет ни одного ответа**. С первым ответом ссылка «Редактировать» пропадает: иначе вопрос и варианты можно было бы подменить под уже проголосовавшую аудиторию.

**Отложено:** логика блока «Поделиться в соцсетях» (пока только вёрстка), UI переголосования (замена ответа в логике заложена, контрола нет), создание категорий.
...
```tsx
{
  ...
  options: {
    categoryDataSource: "em.vp_categories",
    questionDataSource: "em.vp_questions",
    variantDataSource: "em.vp_variants",
    answerDataSource: "em.vp_answers",
    expandable: true,
    expanded: true
  },
  children: [
