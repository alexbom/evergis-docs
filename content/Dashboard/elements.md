...
**Колонка вложений (`subType: "Attachments"`).** Ветка срабатывает раньше разбора типа: тип у атрибута строковый, вложениями его делает уточнение в схеме, и без этого в ячейке оказался бы сырой JSON. Файлы показываются списком (`AttachmentsList` из [[containers#AttachmentContainer|контейнера вложений]]): значок типа файла и имя, клик открывает `Preview` со скачиванием. С `editMode` контейнера и `isEditable` атрибута появляются удаление и добавление по ссылке (`AttachmentLinkDialog`); загрузки с диска нет — она требует ресурса-родителя и файлового api, которых у колонки таблицы нет. В черновик значение кладётся строкой JSON, а не массивом: тип ячейки не должен меняться от самой правки.
